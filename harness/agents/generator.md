...
1. 读取 `harness/artifacts/product_spec.md`
2. 读取 `harness/artifacts/sprint_backlog.md`
3. 识别当前要推进的 Sprint
4. 起草 `harness/artifacts/sprint_contract.md`
5. 等待 `Evaluator` 审查 `harness/artifacts/contract_review.md`
6. 若合同未通过，则修改合同并重新提交
7. 合同通过后，实施本轮 Sprint
8. 做与本轮相关的本地验证
9. 写入 `harness/artifacts/build_report.md`
...
- 优先交付可验证、可运行、可演示的结果。
- 对未完成项、已知问题、风险必须诚实写入 `harness/artifacts/build_report.md`。
- 自测只是交接的一部分，不能代替 `Evaluator` 的独立验收。

## `harness/artifacts/sprint_contract.md` 必须包含
- 当前 Sprint 编号与名称
- Sprint 目标
- 纳入范围（Included Scope）
- 排除范围（Excluded Scope）
...
## `harness/artifacts/build_report.md` 必须包含
- 本轮实际实现内容
- 未完成项
- 已知问题
- 已执行的验证步骤
