...
## Sprint 4
### 目标
- 完成可观测性、README 与简历化叙事收尾，把项目变成“可讲、可演示、可写进简历”的成品。

### Included Scope
- Observability 页面增强（偏演示与讲解）：至少能展示一次运行的 `steps`（来自 Playground 最近一次运行结果），并提供可复制/可下载的诊断信息。
- 运行态与集成可见性（不做大后端链路）：展示 MCP 运行状态（已有接口可用则直接调用）、RAG 标签概览（已有接口可用则直接调用）。
- README 重写：用“产品叙事 + 能力地图 + North Star Demo + 后端模块边界 + 本地运行方式 + 面试讲解点”组织内容。
- 演示脚本与项目亮点整理：沉淀 3~5 分钟讲解脚本（可独立成文档或 README 附录）。

### Excluded Scope
- 不新增与主线无关功能。
- 不做新的后端模块化/大重构。
- 不引入新的前端框架或设计系统迁移。
- 不强制接入完整 trace/tool-call 链路（以 `steps` 与运行态可见性为主）。
...
- 适合对外演示。
- 适合简历与面试表达。
- Observability 页面不再是纯占位：能够“拿到一段运行证据并讲清楚”。
- README 能让陌生人 3 分钟内理解项目亮点与演示方式。
