...
public class FixedExecuteStrategy implements IExecuteStrategy {
    ...
    public String execute(ExecuteContext context, SseEmitter emitter) throws Exception {
        ...
        for (Map.Entry<String, String> entry : clientTypeMap.entrySet()) {
            ...
            StrategyHelper.sendStageEvent(emitter, "execution", "active", step, totalSteps, null, context.getConversationId());
            ...
            // 推送SSE事件
            StrategyHelper.sendStageEvent(emitter, "execution", "done", step, totalSteps, lastOutput, context.getConversationId());
        ...
        StrategyHelper.sendStageEvent(emitter, "complete", "done", 0, totalSteps, "执行完成", context.getConversationId());
        return lastOutput;
    }
